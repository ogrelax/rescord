 ...
 *   2. Inline DOM handlers (onclick/oninput/...) only call functions that exist.
...
if (html) {
  ...
  const handlerRe = /\b(?:onclick|oninput|onchange|onkeydown|onkeyup|oncontextmenu|onsubmit|onmousedown|onmouseup)\s*=\s*"([^"]*)"/g;
  let handlerCount = 0, bad = 0;
  for (const m of html.matchAll(handlerRe)) {
    handlerCount++;
    const expr = m[1];
    for (const c of expr.matchAll(/(^|[^.\w$])([A-Za-z_$][\w$]*)\s*\(/g)) {
      const name = c[2];
      if (!defined.has(name) && !SAFE.has(name)) { err(`handler calls undefined function: ${name}()  ${C.d}[${expr.slice(0, 48)}]${C.x}`); bad++; }
    }
  }
  if (!bad) ok(`all ${handlerCount} inline handlers resolve to defined functions`);
}

// ---- 4. Referenced asset existence ----------------------------------------
head('Static assets');
for (const a of [
...
// ---- 3. DeepFilterNet3 asset integrity ------------------------------------
(async () => {
